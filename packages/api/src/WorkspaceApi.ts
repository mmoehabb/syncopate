...
import type { WorkspaceWithBoards, CreateWorkspacePayload } from "@syncopate/types";
...
export class WorkspaceApi extends ApiClient {
  ...
  public async createWorkspace(payload: CreateWorkspacePayload): Promise<Workspace> {
    const response = await this.post<{ workspace: Workspace }>("", payload);
    return response.data.workspace;
  }

  public async deleteWorkspace(workspaceName: string): Promise<{ message: string }> {
