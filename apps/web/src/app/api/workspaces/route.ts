...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!name) {
      return apiError(API_ERRORS.customBadRequest("Workspace name is required"));
    }
    ...
    if (!userSubscription) {
      return apiError(API_ERRORS.customForbidden("Active subscription required to create a workspace"));
    }
    ...
    if (maxWorkspaces !== -1 && currentWorkspacesCount >= maxWorkspaces) {
      return apiError(API_ERRORS.customForbidden(`You have reached your limit of ${maxWorkspaces} workspaces on this plan.`));
    }
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    if (workspace.members.length === 0 || workspace.members[0].role !== "ADMIN") {
      return apiError(API_ERRORS.customForbidden("Unauthorized to delete this workspace"));
    }
