  ...
  help: {
    ...
    action: ({ printOutput }) => {
      const output = [
        '--- SYNC-OS v1.0.0 ---',
        'Available Commands:',
        ...Object.values(COMMAND_REGISTRY).map(cmd => `  /${cmd.name.padEnd(10)} - ${cmd.description}`),
        '',
        'Normal Mode Shortcuts:',
        ...Object.values(NORMAL_ACTIONS_REGISTRY).map(act => `  ${act.key.padEnd(11)} - ${act.description}`)
      ];
      printOutput(output);
    }
  },
